...
/*!
 * \file i2c_twoBoards_comPolling.c
...
uint8_t aTxBuffer1[] = " ****I2C TwoBoards communication based on Polling running on CPU1**** ";
/*! \brief Buffer used by CPU2 for transmission */
uint8_t aTxBuffer2[] = " ****I2C TwoBoards communication based on Polling running on CPU2**** ";
...
int main(void)
{
    ...
    while (I2C_masterTransmit(pi2cHandle, aTxBuffer, TXBUFFERSIZE, 10000) != kSTATUS_I2C_OK)
    {
        /* Processing Error */
        if (pi2cHandle->errorCode != kI2C_ABRT_7B_ADDR_NOACK)
        {
            Error_Handler();
        }
    }
    ...
    while (I2C_masterReceive(pi2cHandle, aRxBuffer, RXBUFFERSIZE, 10000) != kSTATUS_I2C_OK)
    {
        /* Processing Error */
        if (pi2cHandle->errorCode != kI2C_ABRT_7B_ADDR_NOACK)
        {
    ...
    /* Put I2C peripheral in reception process */
    if (I2C_slaveReceive(pi2cHandle, (uint8_t *)aRxBuffer, RXBUFFERSIZE, 10000) != kSTATUS_I2C_OK)
    {
    ...
    if (I2C_slaveTransmit(pi2cHandle, (uint8_t *)aTxBuffer, TXBUFFERSIZE, 10000) != kSTATUS_I2C_OK)
    {
