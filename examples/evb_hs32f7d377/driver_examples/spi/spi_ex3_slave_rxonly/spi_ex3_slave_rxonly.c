...
#define SPI_RECEIVE_DATA_LENS  (8U)      /*!< 8 bytes*/
...
int main(void)
{
    ...
    mySPI->init.mode           = SPI_MODE_SLAVE;
    mySPI->init.direction      = SPI_DIRECTION_RXONLY;
    mySPI->init.selectToggleEn = SPI_SSTE_ENABLE;
    ...
    rxLens = SPI_RECEIVE_DATA_LENS;

    /*  Wait receive 8 Bytes */
    if (status != SPI_receive(mySPI, spiRxBuff, rxLens, SPI_TRANSMIT_TIMEOUT))
    {
        /* Transmit error */
        while (1)
        {
