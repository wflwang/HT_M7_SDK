...
const uint8_t s_uartExampleString[] = "\r\n"
                                      "UART RS485 Full Duplex Interrupt Callback Example:\r\n"
...
int main(void)
{
    ...
    (void)UART_RS485FD_init(EXAMPLE_UART, &config, EXAMPLE_UART_CLOCK);
    (void)UART_RS485FD_registerCallback(EXAMPLE_UART, UART_userCallback, 0U);

    /* Send the example banner */
    (void)UART_RS485FD_writeDataNonBlocking(EXAMPLE_UART, s_uartExampleString, sizeof(s_uartExampleString));
    s_transOnGoing = true;
    ...
    while (1)
    {
        ...
        {
            (void)UART_RS485FD_readDataNonBlocking(EXAMPLE_UART, s_rxBuffer, sizeof(s_rxBuffer));
            s_transOnGoing = true;
        }
        ...
        {
            (void)UART_RS485FD_writeDataNonBlocking(EXAMPLE_UART, s_txBuffer, sizeof(s_txBuffer));
            s_transOnGoing = true;
        }
    }
}
