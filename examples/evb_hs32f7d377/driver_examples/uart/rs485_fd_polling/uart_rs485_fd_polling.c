...
const uint8_t s_uartExampleString[] = "\r\n"
                                      "UART RS485 Full Duplex Polling Example:\r\n"
...
int main(void)
{
    ...
    (void)UART_RS485FD_writeDataBlocking(EXAMPLE_UART, s_uartExampleString, sizeof(s_uartExampleString));

    while (1)
    {
        (void)UART_RS485FD_readDataBlocking(EXAMPLE_UART, &ch, sizeof(ch));
        (void)UART_RS485FD_writeDataBlocking(EXAMPLE_UART, &ch, sizeof(ch));
    }
}
