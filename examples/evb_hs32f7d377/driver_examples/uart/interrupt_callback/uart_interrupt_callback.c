...
const uint8_t s_uartExampleString[] = "\r\n"
                                      "UART Interrupt Callback Example:\r\n"
...
/*! \brief Flag to show if there is a transmit operation under processing */
volatile bool s_txOnGoing = false;
...
/*! \brief Flag to show if there is a receive operation under processing */
volatile bool s_rxOnGoing = false;
...
int main(void)
{
    ...
    (void)UART_registerCallback(EXAMPLE_UART, UART_userCallback, 0U);

    /* Send the example banner */
    (void)UART_writeDataNonBlocking(EXAMPLE_UART, s_uartExampleString, sizeof(s_uartExampleString));
    s_txOnGoing = true;

    while (1)
    {
        if ((!s_rxOnGoing) && (!s_rxBufferFull))
        {
            (void)UART_readDataNonBlocking(EXAMPLE_UART, s_rxBuffer, sizeof(s_rxBuffer));
            s_rxOnGoing = true;
        }
        ...
        {
            (void)UART_writeDataNonBlocking(EXAMPLE_UART, s_txBuffer, sizeof(s_txBuffer));
            s_txOnGoing = true;
        }
    }
}
