...
void PosSpeed_calculate(PosSpeed_Object *p, uint32_t *c)
{
    ...
    _iq temp2 = 0, newPosCnt, oldPosCnt;
     ...
     */
    p->directionQEP = EQEP_getDirection(EQEP1);

    /* Capture position once per QA/QB period */
    pos16bVal = (uint16_t)EQEP_getPosition(EQEP1);
    ...
    if((EQEP_getInterruptStatus(EQEP1) & EQEP_INT_INDEX_EVNT_LATCH) != 0U)
    {
        EQEP_clearInterruptStatus(EQEP1, EQEP_INT_INDEX_EVNT_LATCH);
    }
    ...
    if((EQEP_getInterruptStatus(EQEP1) & EQEP_INT_UNIT_TIME_OUT) != 0)
    {
        ...
        pos16bVal = (uint16_t)EQEP_getPositionLatch(EQEP1);
        ...
        newPosCnt = temp/32768.0;/*_IQ15toIQ(temp); */
        oldPosCnt = p->oldPos;
         ...
         */
        p->speedRPMFR = (int16_t)_IQmpy(p->baseRPM, p->speedFR);
    ...
    if((EQEP_getStatus(EQEP1) & EQEP_STS_UNIT_POS_EVNT) != 0)
    {
        ...
        {
            temp1 = (uint32_t)EQEP_getCapturePeriodLatch(EQEP1);
        }
        else
        {
        ...
        {
            /* Q0 = Q0 * GLOBAL_Q => _IQXmpy(), X = GLOBAL_Q */
            p->speedRPMPR = (int16_t)-_IQmpy(p->baseRPM, p->speedPR);
        }
