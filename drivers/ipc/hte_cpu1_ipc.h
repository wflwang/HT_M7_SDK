...
/*!
 * \file hte_cpu1_ipc.h
 * \brief SDK CPU1 IPC driver header file
 * \version 1.0.0
 */

#ifndef HTE_CPU1_IPC_H
#define HTE_CPU1_IPC_H
...
/*!
 * \addtogroup DRV_HTE_CPU1_IPC CPU1 IPC Driver
 *
 * \brief SDK CPU1 IPC driver
 *
 * @{
 */
 ...
 * Definitions
 ******************************************************************************/
#define IPC_BOOTMODE_CPU2_WAIT      0x5A00 /*!< CPU1 set cpu2 boot wait */
#define IPC_BOOTMODE_CPU2_EFLASH    0x5A01 /*!< CPU1 set cpu2 boot from eflash */
#define IPC_BOOTMODE_CPU2_ITCM      0x5A02 /*!< CPU1 set cpu2 boot from itcm */
#define IPC_BOOTMODE_CPU2_SRAM      0x5A03 /*!< CPU1 set cpu2 boot from sram */
#define IPC_BOOTMODE_CPU2_WAIT_ADDR 0x5A04 /*!< CPU1 set cpu2 boot from cmd */
#define IPC_BOOTMODE_CPU2_RESET     0x5A05 /*!< CPU1 set cpu2 boot reset */
#define IPC_BOOTMODE_CPU2_IGNORE    0x5A06 /*!< CPU1 set cpu2 boot ignore */
...
{
    IPC_REGS_CPU1->IPCACK = (0x1UL << ipcNum);
}
...
{
    return ((IPC_REGS_CPU1->IPCSTS & (0x1UL << ipcNum)) != 0U);
}
...
{
    IPC_REGS_CPU1->IPCSET = (0x1UL << ipcNum);
}
...
{
    IPC_REGS_CPU1->IPCCLR = (0x1UL << ipcNum);
}
...
{
    return ((IPC_REGS_CPU1->IPCFLG & (0x1UL << ipcNum)) != 0U);
}
...
{
    while ((IPC_REGS_CPU1->IPCSTS & (0x1UL << ipcNum)) == 0U)
    {
    }
}
...
{
    while ((IPC_REGS_CPU1->IPCFLG & (0x1UL << ipcNum)) != 0U)
    {
    }
}
...
static inline uint64_t IPC_getCounter(void)
{
    ...
    //
    uint32_t ctrL = IPC_REGS_CPU1->IPCCOUNTERL;
    uint32_t ctrH = IPC_REGS_CPU1->IPCCOUNTERH;
...
{
    IPC_REGS_CPU1->IPCLOCALREPLY = data;
}
...
{
    return (IPC_REGS_CPU1->IPCREMOTEREPLY);
}

/*!
 * \brief Set boot mode
 *
 * \details This function set boot mode to send from local to remote
 *
 * \param[in] mode is bootmode value
 *
 */
static inline void IPC_setBootMode(uint32_t mode)
{
    IPC_REGS_CPU1->IPCBOOTMODE = mode;
}

/*!
 * \brief Get boot status
 *
 * \details This function get boot status from remote.
 *
 * \return Return boot status value.
 *
 */
static inline uint32_t IPC_getBootStatus(void)
{
    return IPC_REGS_CPU1->IPCBOOTSTS;
}
...
/*!
 * @}
 */ /* end of DRV_HTE_CPU1_IPC */

#endif /* HTE_CPU1_IPC_H */
