...
/*!
 * \file hte_cpu1_ipc.c
 * \brief SDK CPU1 IPC driver source file
 * \version 1.0.0
 */
...
bool IPC_sendCommand(IPC_Id_t index, uint32_t command, uint32_t addr, uint32_t data)
{
    ...
    if ((IPC_REGS_CPU1->IPCFLG & (0x1UL << index)) == 0x0UL)
    {
        ret                        = true;
        IPC_REGS_CPU1->IPCSENDCOM  = command;
        IPC_REGS_CPU1->IPCSENDDATA = data;
        IPC_REGS_CPU1->IPCSENDADDR = addr;
        IPC_REGS_CPU1->IPCSET      = (0x1UL << index);
    }
...
bool IPC_readCommand(IPC_Id_t index, uint32_t *command, uint32_t *addr, uint32_t *data)
{
    ...
    if ((IPC_REGS_CPU1->IPCSTS & (0x1UL << index)) != 0x0UL)
    {
        ret      = true;
        *command = IPC_REGS_CPU1->IPCRECVCOM;
        *data    = IPC_REGS_CPU1->IPCRECVDATA;
        *addr    = IPC_REGS_CPU1->IPCRECVADDR;
    }
